...
export class ProfileDropdownComponent implements OnInit {
  ...
  ngOnInit() {
    this.accountsService.user.subscribe(value => {
      this.image = value.user.avatar
        ? value.user.avatar
        : this.profileInitials.createImageFromInitials();
  ...
  onDocumentClick(event: Event) {
    const isClickedOutside = !(event.target as HTMLElement).closest(
      '.profileMenu',
    );
    const isProfileButton = !(event.target as HTMLElement).closest(
      '.profileButton',
    );
