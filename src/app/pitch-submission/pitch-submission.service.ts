...
import { messageData, pitchData, startupData } from '../models/interfaces';
...
export class PitchSubmissionService {
  ...
  constructor(private http: HttpClient) {
    this.pitchSubject = new BehaviorSubject(
  ...
  getIndustries() {
    return this.http.get<messageData>(
      `${environment.BACKEND_URL}/api/getAllIndustries`,
    );
  }
  ...
  public get startupProfileValid() {
    const { startupName, industry, stage, country, region, city } =
      this.pitchData;
  ...
  public get pitchDetailsValid() {
    const { amountToRaise, raisedAmount, purpose, equity, startupBio } =
      this.pitchData;
  ...
  submitPitch() {
    return this.http.post<messageData>(
      `${environment.BACKEND_URL}/api/pitch-submission`,
      this.pitchFormData,
    );
  }
